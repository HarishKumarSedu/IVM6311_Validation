...
from procedures.startup import StartupAzComp
...
class AzComp:
    ...
    def Azcomp_tests(self, chipid=1):
        ...
        #####################################OUTN_EXT_PRT
        print('OUTN_EXT_PRT TEST>')
        print('Force voltage on OUTP and OUTN and measure differential voltage on those pins')
        OUTN_EXT_PRT=[]
        self.mcp.mcpWrite(SlaveAddress=0x6c, data=[0x1A,0x0B])
        sleep(1)
        self.PowerSupply.emulMode_2Q(3)
        self.PowerSupply.setVoltage(channel=4,voltage=2)
        ...
        os.makedirs(f'measurements/Char/AzComp/{self.file}',exist_ok=True)
        if not os.path.exists(f'measurements/Char/AzComp/{self.file}/AzComp.json'):
            with open(f'measurements/Char/AzComp/{self.file}/AzComp.json', 'w') as file:
                json.dump({},file)
        with open(f'measurements/Char/AzComp/{self.file}/AzComp.json', 'r') as file :
            data = json.load(file)
